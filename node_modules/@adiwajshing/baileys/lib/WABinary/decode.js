...
const decompressingIfRequired = (buffer) => {
    if (2 & buffer.readUInt8()) {
        buffer = zlib_1.inflateSync(buffer.slice(1));
    }
...
const decodeDecompressedBinaryNode = (buffer, opts, indexRef = { index: 0 }) => {
    ...
    const readList = (tag) => {
        ...
        for (let i = 0; i < size; i++) {
            items.push(exports.decodeDecompressedBinaryNode(buffer, opts, indexRef));
        }
        return items;
    };
...
const decodeBinaryNode = (buff) => {
    const decompBuff = exports.decompressingIfRequired(buff);
    return exports.decodeDecompressedBinaryNode(decompBuff, constants);
};
